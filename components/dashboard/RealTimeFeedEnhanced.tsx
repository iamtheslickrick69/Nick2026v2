...
export default function RealTimeFeedEnhanced() {
  const { feedFilters, selectedDepartment, setFeedbackDetailPanel } = useDashboardStore()
  ...
  const filteredFeedback = useMemo(() => {
    ...
    if (feedFilters.source !== "all") {
      filtered = filtered.filter((item) => item.source === feedFilters.source)
    }

    // Filter by sentiment
    if (feedFilters.sentiment !== "all") {
      filtered = filtered.filter((item) => item.sentiment === feedFilters.sentiment)
    }

    // Filter by priority
    if (feedFilters.priority !== "all") {
      filtered = filtered.filter((item) => item.priority === feedFilters.priority)
    }

    // Filter by selected department (cross-component linking)
    if (selectedDepartment) {
      filtered = filtered.filter(
        (item) => item.department === selectedDepartment || item.segment === selectedDepartment
      )
    }

    return filtered
  }, [feedFilters, selectedDepartment])
  ...
  const handleFeedbackClick = (item: any) => {
    setFeedbackDetailPanel({
      ...
      anonymityLevel: item.anonymityLevel,
      tags: item.tags,
    })
  }
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-2xl border-2 shadow-sm hover:shadow-lg transition-all flex flex-col h-full ${
        selectedDepartment ? "border-blue-400 ring-2 ring-blue-100" : "border-[#E5E5E5]"
      }`}
    >
      {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#666666]">
                {filteredFeedback.length} of {recentFeedback.length} messages
              </p>
            </div>
          </div>
          <motion.div
        ...
        <AnimatePresence mode="popLayout">
          {filteredFeedback.length === 0 ? (
            ...
            >
              ...
              <h4 className="text-lg font-semibold text-[#202020] mb-2">No feedback found</h4>
              <p className="text-sm text-[#666666] max-w-xs">Try adjusting your filters to see more results.</p>
            </motion.div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      {/* Source Icon */}
                      <div
                        className={`w-6 h-6 rounded-lg bg-gradient-to-br ${sourceColor} flex items-center justify-center flex-shrink-0`}
                      >
                        {isEmployee ? (
                  ...
                  {item.tags && item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {item.tags.map((tag) => (
                        <span key={tag} className="px-2 py-0.5 rounded-full bg-[#F5F3F0] text-[10px] text-[#666666] font-medium">
                          #{tag}
                        </span>
                      ))}
                    </div>
